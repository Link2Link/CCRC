...
x_log = x0;
t_log = 0;
u_log = zeros(size(control{1}, 1), 1);
...
for t = 0:dt:T
    ...
    if git_flag
        imwrite(imind,cm,'rename.gif','gif','LoopCount',Inf,'DelayTime',dt/100);
        git_flag = 0;
    else
        imwrite(imind,cm,'rename.gif','gif','WriteMode','append' ,'DelayTime',dt/100);
    end
    pause(dt/10);
end
